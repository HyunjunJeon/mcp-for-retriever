...
async def search_all_tool(query: str, limit: int = 10) -> Dict[str, Any]:
    """
    ...
    if not retrievers:
        return {"status": "error", "error": "No retrievers available"}
    ...
    # 동시에 모든 검색 실행
    tasks = [search_source(name, retriever) for name, retriever in retrievers.items()]
