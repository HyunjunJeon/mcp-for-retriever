...
def _validate_auth_settings(config: ServerConfig) -> List[str]:
    ...
    if auth.internal_api_key:
        ...
        if (
            auth.internal_api_key == "your-key"
            or "test" in auth.internal_api_key.lower()
        ):
...
def _validate_security(config: ServerConfig) -> List[str]:
    ...
    is_production = (
        ...
        or config.name.endswith("-prod")
        or "production" in config.name.lower()
    )
...
def validate_profile_compatibility(
    current_profile: ServerProfile, target_profile: ServerProfile
) -> Tuple[bool, List[str]]:
    """
    프로파일 간 호환성 검증
    ...
    current_idx = (
        profile_order.index(current_profile) if current_profile in profile_order else -1
    )
    target_idx = (
        profile_order.index(target_profile) if target_profile in profile_order else -1
    )
    ...
    # 특정 전환 경고
    if (
        current_profile == ServerProfile.CACHED
        and target_profile != ServerProfile.COMPLETE
    ):
        warnings.append("캐시된 데이터가 손실될 수 있음")

    if (
        current_profile == ServerProfile.CONTEXT
        and target_profile == ServerProfile.BASIC
    ):
