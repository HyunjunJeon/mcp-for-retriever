...
class RetrieverConfig:
    ...
    postgres_dsn: str = "postgresql://mcp_user:mcp_password@localhost:5432/mcp_retriever"
...
class ServerConfig:
    ...
    def validate(self) -> tuple[bool, list[str]]:
        ...
        if self.features["auth"] and self.auth_config:
            if not self.auth_config.internal_api_key and self.auth_config.require_auth:
                errors.append("인증이 활성화되었지만 MCP_INTERNAL_API_KEY가 설정되지 않음")
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "rate_limit_config": self.rate_limit_config.__dict__ if self.rate_limit_config else None,
            "logging_config": self.logging_config.__dict__ if self.logging_config else None,
            "retriever_config": self.retriever_config.__dict__ if self.retriever_config else None,
        }