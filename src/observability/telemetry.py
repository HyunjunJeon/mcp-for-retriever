...
class TelemetrySetup:
    ...
    def _instrument_libraries(self):
        ...
        FastAPIInstrumentor().instrument(
            tracer_provider=self._tracer_provider, excluded_urls="/health,/metrics"
        )
        ...
        # Instrument AsyncPG
        AsyncPGInstrumentor().instrument(tracer_provider=self._tracer_provider)

        # Instrument Redis
        RedisInstrumentor().instrument(tracer_provider=self._tracer_provider)
    ...
    def create_custom_metrics(self):
        ...
        self.request_duration = meter.create_histogram(
            name="mcp.request.duration", description="MCP request duration", unit="ms"
        )

        # Active users gauge
        self.active_users = meter.create_up_down_counter(
            name="mcp.users.active", description="Number of active users", unit="1"
        )
