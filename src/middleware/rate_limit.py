...
class RateLimitMiddleware:
    ...
    def __init__(
        ...
        # In-memory storage for simple rate limiting (not recommended for production)
    ...
    def _get_user_identifier(self, user: Any) -> str:
        ...
        if isinstance(user, dict):
            ...
            return str(user.get("id") or user.get("email") or user.get("ip", "anonymous"))
        return "anonymous"
    ...
    async def _check_memory_rate_limit(self, user_id: str) -> tuple[bool, Optional[int]]:
    ...
    async def get_usage_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            bucket = self._get_token_bucket(user_id) if user_id in self._token_buckets else None
