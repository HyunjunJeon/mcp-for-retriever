...
class ErrorHandlerMiddleware(Middleware):
    ...
    def __init__(
        ...
        # Error statistics
        self._error_counts = {"total": 0, "by_type": {}, "by_method": {}}
    ...
    async def _handle_response_error(
        ...
        self._error_counts["by_method"][method] = (
            self._error_counts["by_method"].get(method, 0) + 1
        )
    ...
    async def _log_mcp_error(
        self, error: MCPError, request_id: str, method: str, user: Dict[str, Any]
    ):
    ...
    async def _log_unexpected_error(
        self, error: Exception, request_id: str, method: str, user: Dict[str, Any]
    ):
        ...
        if self.capture_stack_trace:
            ...
            if len(stack_trace) > self.max_error_log_length:
                stack_trace = (
                    stack_trace[: self.max_error_log_length] + "... [TRUNCATED]"
                )
            error_context["stack_trace"] = stack_trace

        # Update statistics
        self._error_counts["total"] += 1
        self._error_counts["by_type"]["UnexpectedError"] = (
            self._error_counts["by_type"].get("UnexpectedError", 0) + 1
        )
        self._error_counts["by_method"][method] = (
            self._error_counts["by_method"].get(method, 0) + 1
        )

        logger.exception("Unexpected error in request processing", **error_context)
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            if self._error_counts["by_method"]
            else None,
        }
