...
class ObservabilityMiddleware:
    ...
    def __init__(
        ...
        self.tracer = (
            get_tracer(f"{service_name}.middleware") if enable_tracing else None
        )
    ...
    async def __call__(
        ...
        try:
            ...
            if span:
                ...
                if error_occurred:
                    ...
                    if error_details and isinstance(error_details, dict):
                        ...
                        span.set_attribute(
                            "error.message", error_details.get("message", "unknown")
                        )
                else:
            ...
            if "sentry_transaction" in locals() and sentry_transaction:
                sentry_transaction.set_status(
                    "ok" if not error_occurred else "internal_error"
                )
    ...
    def inject_trace_context(self, headers: Dict[str, str]):
        ...
        if span and span.is_recording():
            ...
            headers["traceparent"] = (
                f"00-{format(context.trace_id, '032x')}-{format(context.span_id, '016x')}-01"
            )
