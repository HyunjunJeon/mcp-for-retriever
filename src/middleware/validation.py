...
class ValidationMiddleware:
    ...
    def __init__(
        ...
        self.tool_permissions = tool_permissions or {
            "admin": {"search_web", "search_vectors", "search_database", "search_all", "health_check"},
            "user": {"search_web", "search_vectors", "search_database", "search_all", "health_check"},
    ...
    async def __call__(self, request: Dict[str, Any], call_next: Callable) -> Dict[str, Any]:
        ...
        if request.get("jsonrpc") != "2.0":
            return self._invalid_request_response("Invalid jsonrpc version, must be 2.0")
    ...
    async def _validate_tool_call(self, request: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Validate tool call permissions."""
        params = request.get("params", {})
        
        if not isinstance(params, dict):
            return self._invalid_params_response("Params must be an object for tools/call")
        ...
        user_roles = user.get("roles", ["guest"]) if isinstance(user, dict) else ["guest"]
        ...
        if tool_name not in allowed_tools:
            ...
            return self._permission_denied_response(f"Access denied for tool: {tool_name}")
    ...
    def _validate_params(self, request: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if method == "tools/call":
            ...
            tool_name = params.get("name")
            if tool_name in ["search_web", "search_vectors", "search_database", "search_all"]:
                if "query" not in arguments:
                    return self._invalid_params_response(f"Missing required argument 'query' for {tool_name}")
                ...
                if len(query) > 1000:
                    return self._invalid_params_response("Query too long (max 1000 characters)")
            
            # Validate search_vectors specific params
            if tool_name == "search_vectors" and "collection" not in arguments:
                return self._invalid_params_response("Missing required argument 'collection' for search_vectors")
