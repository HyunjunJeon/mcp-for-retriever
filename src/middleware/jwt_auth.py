...
class JWTAuthMiddleware(Middleware):
    ...
    async def on_message(self, context: MiddlewareContext[Any], call_next) -> Any:
        """
        ...
        if not self.require_auth and method in self.skip_auth_methods:
            logger.debug("인증 건너뛰기", method=method, reason="skip_auth_methods")
            # 익명 사용자 정보 설정
        ...
        if not auth_header:
            if self.require_auth:
                logger.warning(
                    "인증 헤더 누락", method=method, require_auth=self.require_auth
                )
                raise McpError(
                    ErrorData(code=-32603, message="Missing authorization header")
                )
            else:
                # 익명 접근 허용
        ...
        if not auth_header.startswith("Bearer "):
            ...
            raise McpError(
                ErrorData(code=-32603, message="Invalid authorization header format")
            )
        ...
        try:
            ...
            if token_data is None:
                ...
                raise McpError(
                    ErrorData(code=-32603, message="Invalid or expired JWT token")
                )
            ...
            raise McpError(
                ErrorData(code=-32603, message="Internal authentication error")
            )

    async def _extract_auth_header(
    ...
    async def _set_anonymous_user_context(
        self, context: MiddlewareContext[Any]
    ) -> None:
        """
        익명 사용자 컨텍스트 설정
    ...
    async def _set_service_user_context(self, context: MiddlewareContext[Any]) -> None:
        ...
        try:
            ...
            raise McpError(
                ErrorData(code=-32603, message="Failed to set service user context")
            )

    async def _set_jwt_user_context(
        self, context: MiddlewareContext[Any], token_data: TokenData
    ) -> None:
        """
        ...
        try:
            ...
            raise McpError(
                ErrorData(code=-32603, message="Failed to set JWT user context")
            )
...
class AuthorizationMiddleware(Middleware):
    ...
    async def on_call_tool(self, context: MiddlewareContext, call_next) -> Any:
        ...
        if not user_info:
            ...
            raise McpError(
                ErrorData(
                    code=-32603, message="Authentication required for tool execution"
                )
            )
        ...
        if not self.rbac_service.check_tool_permission(user_roles, tool_name):
            ...
            raise McpError(
                ErrorData(
                    code=-32603,
                    message=f"Access denied: insufficient permissions for tool '{tool_name}'",
                )
            )
    ...
    async def on_list_tools(self, context: MiddlewareContext, call_next) -> Any:
        ...
        for tool in all_tools:
            ...
            if tool_name and self.rbac_service.check_tool_permission(
                user_roles, tool_name
            ):
