...
class UnifiedMCPServer:
    ...
    def __init__(self, config: ServerConfig):
        ...
        self.jwt_auth_middleware = (
            None  # Removed - using FastMCP BearerAuthProvider instead
        )
    ...
    def _init_components(self):
        ...
        if self.config.features["error_handler"]:
            include_details = (
                ...
                and self.config.logging_config.log_level == "DEBUG"
            )
        ...
        if self.config.features["auth"] and self.config.auth_config:
            ...
            self.jwt_service = JWTService(
                ...
                refresh_token_expire_minutes=self.config.auth_config.jwt_refresh_token_expire_days
                * 24
                * 60,
            )
            ...
            logger.debug(
                "JWT 서비스 및 BearerVerifier 초기화 완료 (미들웨어는 FastMCP 표준으로 대체)"
            )
        ...
        if self.config.features["rate_limit"] and self.config.rate_limit_config:
            ...
            if self.config.features["cache"] and self.config.cache_config:
                try:
                    ...
                    # Redis 연결은 async lifecycle에서 처리됨
                    # 여기서는 Redis URL에서 직접 redis 클라이언트 생성
                    import redis.asyncio as redis

                    redis_client = redis.from_url(
                        self.config.cache_config.redis_url, decode_responses=True
                    )
                    logger.info("Rate limiter에 Redis 클라이언트 설정")
                except Exception as e:
                    logger.warning(f"Rate limiter Redis 클라이언트 생성 실패: {e}")
        ...
        if self.config.features["metrics"]:
            self.metrics_middleware = MetricsMiddleware(
                enable_detailed_metrics=True, metrics_window_seconds=3600
            )
    ...
    async def init_retrievers(self) -> List[str]:
        ...
        try:
            ...
            if config["api_key"]:
                ...
                logger.info(
                    "Tavily 리트리버 초기화 완료", cached=self.config.features["cache"]
                )
            else:
        ...
        try:
            ...
            logger.info(
                "PostgreSQL 리트리버 초기화 완료", cached=self.config.features["cache"]
            )
        except Exception as e:
        ...
        try:
            ...
            logger.info(
                "Qdrant 리트리버 초기화 완료", cached=self.config.features["cache"]
            )
        except Exception as e:
    ...
    def create_server(self) -> FastMCP:
        ...
        if self.config.features["auth"] and self.bearer_verifier:
            logger.info(
                "FastMCP 커스텀 JWT 인증 활성화 - 도구 함수 레벨 AccessToken 의존성 주입"
            )
...
"""
    ...
    def _create_context_middleware(self):
        ...
        async def context_middleware(context, call_next):
            ...
            logger.info(
                "MCP 요청", request_id=request_id, method=method, tool_name=tool_name
            )
    ...
    def _register_tools(self, server: FastMCP):
        ...
        async def search_web(
            ...
            # 사용자 컨텍스트 관리
            await self._get_or_create_user_context(ctx, access_token)

            # 사용자 정보 추출
            user_id, user_email, user_type = self._get_user_info_from_token(
                access_token
            )
            ...
            if "tavily" not in self.retrievers:
                await self._record_tool_usage(
                    ctx, tool_name, start_time, False, "Tavily retriever not available"
                )
            ...
            if not retriever.connected:
                await self._record_tool_usage(
                    ctx, tool_name, start_time, False, "Tavily retriever not connected"
                )
            ...
            try:
                ...
                async for result in retriever.retrieve(
                    query, limit=limit, **search_params
                ):
                ...
                logger.error(
                    "웹 검색 실패",
                    extra={"error": str(e), "user_id": user_id, "user_type": user_type},
                )
                emoji = "❌" if use_emoji else ""
        ...
        async def search_vectors(
            ...
            if "qdrant" not in self.retrievers:
                await self._record_tool_usage(
                    ctx, tool_name, start_time, False, "Qdrant retriever not available"
                )
            ...
            if not retriever.connected:
                await self._record_tool_usage(
                    ctx, tool_name, start_time, False, "Qdrant retriever not connected"
                )
        ...
        async def create_vector_document(
            ...
            try:
                ...
                await retriever.upsert(collection=collection, documents=[document])
        ...
        async def update_vector_document(
            ...
            emoji = "📝" if use_emoji else ""
            await ctx.info(
                f"{emoji} '{collection}' 컬렉션의 문서 '{document_id}' 업데이트 중..."
            )
            ...
            try:
                ...
                # upsert를 사용하여 업데이트
                await retriever.upsert(collection=collection, documents=[update_doc])
        ...
        async def delete_vector_document(
            ctx: Context, collection: str, document_id: str
        ) -> Dict[str, Any]:
            """
            벡터 컬렉션에서 문서 삭제
            ...
            await ctx.info(
                f"{emoji} '{collection}' 컬렉션에서 문서 '{document_id}' 삭제 중..."
            )
            ...
            try:
                ...
                # 단일 ID를 리스트로 변환하여 삭제
                await retriever.delete(collection=collection, ids=[doc_id])
        ...
        async def create_database_record(
            ctx: Context, table: str, data: Dict[str, Any]
        ) -> Dict[str, Any]:
            """
            ...
            if table not in allowed_tables:
                ...
                raise ToolError(
                    f"허용되지 않은 테이블: {table}. 허용된 테이블: {', '.join(allowed_tables)}"
                )

            try:
                # 안전한 SQL 쿼리 생성
                query, values = await retriever.compose_insert_query(
                ...
                if result:
                    ...
                    return {"status": "success", "table": table, "record": result}
                else:
        ...
        async def update_database_record(
            ctx: Context, table: str, record_id: str, data: Dict[str, Any]
        ) -> Dict[str, Any]:
            """
            ...
            emoji = "✏️" if use_emoji else ""
            await ctx.info(
                f"{emoji} '{table}' 테이블의 레코드 '{record_id}' 수정 중..."
            )
            ...
            try:
                ...
                async with retriever.transaction() as conn:
                    ...
                    if result:
                        ...
                        return {"status": "success", "table": table, "record": record}
                    else:
        ...
        async def delete_database_record(
            ...
            emoji = "🗑️" if use_emoji else ""
            await ctx.info(
                f"{emoji} '{table}' 테이블에서 레코드 '{record_id}' 삭제 중..."
            )
            ...
            try:
                ...
                if deleted_id:
                    ...
                    emoji = "⚠️" if use_emoji else ""
                    await ctx.warning(
                        f"{emoji} 삭제할 레코드를 찾을 수 없음: {record_id}"
                    )
        ...
        async def search_database(
            ...
            if "postgres" not in self.retrievers:
                await self._record_tool_usage(
                    ...
                    start_time,
                    False,
                    "PostgreSQL retriever not available",
                )
            ...
            if not retriever.connected:
                await self._record_tool_usage(
                    ...
                    start_time,
                    False,
                    "PostgreSQL retriever not connected",
                )
                raise ToolError(
                    "데이터베이스 검색을 사용할 수 없습니다 - 연결되지 않음"
                )
            ...
            if query.upper().startswith("SELECT"):
                ...
                emoji = "📝" if use_emoji else ""
                await ctx.info(
                    f"{emoji} 텍스트 검색 수행 중 - 테이블: {table or '모든 테이블'}"
                )
        ...
        async def search_all(
                    ...
                    tasks.append(
                        (
                            ...
                            self._search_single_source(
                                name, retriever, query, limit, ctx, user_id, user_type
                            ),
                        )
                    )
            ...
            if not tasks:
                ...
                # 도구 사용 기록 (실패)
                await self._record_tool_usage(
                    ctx, tool_name, start_time, False, "No connected retrievers"
                )

                return {
                    "results": {},
                    "errors": {"all": "사용 가능한 리트리버가 없습니다"},
                }
            ...
            if task_group_failed:
                # 실패한 도구 사용 기록
                await self._record_tool_usage(
                    ctx, tool_name, start_time, False, combined_error
                )
        ...
        async def health_check(
            ...
            try:
                ...
                # 모든 리트리버가 비정상인 경우
                if not any(
                    r.get("connected", False)
                    for r in health_status["retrievers"].values()
                ):
                ...
                emoji = "✅" if use_emoji else ""
                await ctx.info(
                    f"{emoji} 건강 상태 검사 완료: {health_status['status']}"
                )
        ...
        if self.config.features["cache"]:
            ...
            async def invalidate_cache(
                ...
                emoji = "🗑️" if use_emoji else ""
                await ctx.info(
                    f"{emoji} 캐시 무효화 대상: {retriever_name or '모든 리트리버'}"
                )
                ...
                if retriever_name:
                    ...
                    if hasattr(retriever, "invalidate_cache"):
                        ...
                        await ctx.info(
                            f"{retriever_name}에서 {count}개의 캐시 항목 삭제"
                        )
                else:
                    for name, retriever in self.retrievers.items():
                        if hasattr(retriever, "invalidate_cache"):
            ...
            async def cache_stats(ctx: Context) -> Dict[str, Any]:
                ...
                stats = {}
                for name, retriever in self.retrievers.items():
                    if hasattr(retriever, "_cache") and hasattr(
                        retriever, "_use_cache"
                    ):
    ...
    async def _search_single_source(
        ...
        try:
            use_emoji = (
                self.config.logging_config and self.config.logging_config.use_emoji
            )
            emoji = "🔸" if use_emoji else ""
    ...
    async def _get_or_create_user_context(
        self, ctx: Context, access_token: Optional[AccessToken] = None
    ...
    def _update_user_context_from_token(
        self, user_context: UserContext, access_token: AccessToken
    ) -> None:
        ...
        user_data = {
            ...
            "type": "service"
            if access_token.client_id == "internal-service"
            else "user",
        ...
        if request_id and request_id in self.context_store:
            ...
            logger.info(
                ...
                extra={
                    ...
                    "duration_ms": duration_ms,
                    "success": success,
                    "user_id": user_context.user.get("id")
    ...
    def _extract_jwt_claims(
        self, access_token: Optional[AccessToken]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                "JWT claims 파싱 실패",
                extra={"error": str(e), "resource": access_token.resource},
            )
            return {}
