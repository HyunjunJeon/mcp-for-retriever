...
class SQLiteUserRepository(UserRepository):
    ...
    async def get_user_stats(self) -> dict:
        ...
        try:
            # 전체 사용자 수
            total_result = await self.session.execute(select(func.count(UserDB.id)))
            total_users = total_result.scalar() or 0

            # 활성 사용자 수
            active_result = await self.session.execute(
                select(func.count(UserDB.id)).where(UserDB.is_active.is_(True))
            )
            active_users = active_result.scalar() or 0

            # 인증된 사용자 수
            verified_result = await self.session.execute(
                select(func.count(UserDB.id)).where(UserDB.is_verified.is_(True))
            )
            ...
            roles_result = await self.session.execute(
                text(
                    "SELECT role, COUNT(DISTINCT user_id) FROM user_roles GROUP BY role"
                )
            )
