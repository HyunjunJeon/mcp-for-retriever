...
class JWTBearerVerifier:
    ...
    def __init__(
        self, jwt_service: JWTService, internal_api_key: str, require_auth: bool = True
    ):
        """
        JWT Bearer 검증기 초기화
        ...
        logger.info("JWT Bearer Verifier 초기화", require_auth=require_auth)
    ...
    async def verify(self, token: str) -> Optional[AccessToken]:
        ...
        try:
            ...
            logger.error(
                "예상치 못한 토큰 검증 오류", error=str(e), error_type=type(e).__name__
            )
            return None
...
class CompositeVerifier:
    ...
    def __init__(self, verifiers: List[JWTBearerVerifier]):
        ...
        logger.info("복합 토큰 검증기 초기화", verifier_count=len(verifiers))
