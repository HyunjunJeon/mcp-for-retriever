...
class TestResourcePermissions:
    ...
    def test_wildcard_pattern_matching(self):
        ...
        for pattern, resource_name, expected in test_cases:
            ...
            assert result == expected, f"Pattern {pattern} vs {resource_name}: expected {expected}, got {result}"
    ...
    def test_resource_level_permissions(self):
        """리소스 레벨 권한 테스트"""
        # 기본 역할 권한 가져오기
        user_permissions = self.permission_service._get_default_role_permissions(["user"])
    ...
    def test_power_user_permissions(self):
        """Power user 권한 테스트"""
        power_permissions = self.permission_service._get_default_role_permissions(["power_user"])
        ...
        assert has_analytics_write == True
