...
class TestAuthMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_missing_auth_header(self, auth_middleware, mock_request, mock_call_next):
    ...
    @pytest.mark.asyncio
    async def test_internal_api_key_auth(self, auth_middleware, mock_request, mock_call_next):
    ...
    @pytest.mark.asyncio
    async def test_jwt_token_validation_success(self, auth_middleware, mock_request, mock_call_next):
        ...
        with patch.object(auth_middleware.http_client, 'get', return_value=mock_response) as mock_get:
            result = await auth_middleware(mock_request, mock_call_next)
    ...
    @pytest.mark.asyncio
    async def test_jwt_token_validation_failure(self, auth_middleware, mock_request, mock_call_next):
        ...
        with patch.object(auth_middleware.http_client, 'get', return_value=mock_response):
            result = await auth_middleware(mock_request, mock_call_next)
    ...
    @pytest.mark.asyncio
    async def test_auth_gateway_connection_error(self, auth_middleware, mock_request, mock_call_next):
        """Test auth gateway connection failure."""
        mock_request["headers"]["authorization"] = "Bearer jwt-token"
        
        with patch.object(auth_middleware.http_client, 'get', side_effect=httpx.ConnectError("Connection failed")):
            result = await auth_middleware(mock_request, mock_call_next)
