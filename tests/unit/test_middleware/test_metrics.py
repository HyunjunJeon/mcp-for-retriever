...
def metrics_middleware():
    ...
    return MetricsMiddleware(enable_detailed_metrics=True, metrics_window_seconds=3600)
...
class TestMetricsMiddleware:
    ...
    async def test_error_request_metrics(
        self, metrics_middleware, mock_error_call_next
    ):
    ...
    async def test_response_time_distribution(self, metrics_middleware, mock_call_next):
        ...
        request = {"method": "tools/list", "user": {"id": "user123"}}
        ...
        assert any(
            count > 0 for count in metrics["response_time_distribution"].values()
        )
    ...
    @pytest.mark.asyncio
    async def test_recent_errors_tracking(
        self, metrics_middleware, mock_error_call_next
    ):
