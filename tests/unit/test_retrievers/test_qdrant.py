...
class TestQdrantRetrieverConnection:
    ...
    async def test_connect_success(self, qdrant_config, mock_qdrant_client):
        ...
        with patch(
            "src.retrievers.qdrant.QdrantClient", return_value=mock_qdrant_client
        ):
...
class TestQdrantRetrieverSearch:
    ...
    async def test_retrieve_vector_search(
        self, qdrant_config, mock_qdrant_client, mock_embeddings
    ):
        ...
        async for result in retriever.retrieve(
            "Python programming", collection="documents", limit=5
        ):
    ...
    async def test_retrieve_with_score_threshold(
        ...
        results = []
        async for result in retriever.retrieve(
            "test query", collection="test", score_threshold=0.8
        ):
            results.append(result)
...
class TestQdrantRetrieverOperations:
    ...
    async def test_create_collection(self, qdrant_config, mock_qdrant_client):
        ...
        await retriever.create_collection(
            "new_collection", vector_size=768, distance=Distance.COSINE
        )
    ...
    async def test_upsert_vectors(
        ...
        await retriever.upsert(collection="documents", documents=documents)
    ...
    async def test_delete_vectors(self, qdrant_config, mock_qdrant_client):
        ...
        await retriever.delete(collection="documents", ids=["1", "2", "3"])

        mock_qdrant_client.delete.assert_called_once_with(
            collection_name="documents", points_selector=["1", "2", "3"]
        )
