...
class TestTavilyRetrieverConnection:
    ...
    async def test_connect_success(self, tavily_config):
        ...
        with patch.object(
            retriever, "_test_connection", new_callable=AsyncMock
        ) as mock_test:
    ...
    async def test_connect_failure(self, tavily_config):
        ...
        with patch.object(
            ...
            with pytest.raises(
                ConnectionError, match="Failed to connect to Tavily API"
            ):
                await retriever.connect()
...
class TestTavilyRetrieverSearch:
    ...
    async def test_retrieve_with_custom_limit(
        self, tavily_config, mock_tavily_response
    ):
...
class TestTavilyRetrieverIntegration:
    ...
    async def test_context_manager_usage(self, tavily_config, mock_tavily_response):
        ...
        with patch(
            "src.retrievers.tavily.TavilyRetriever._test_connection",
            new_callable=AsyncMock,
        ):
            with patch(
                "src.retrievers.tavily.TavilyRetriever._search", new_callable=AsyncMock
            ) as mock_search:
                mock_search.return_value = mock_tavily_response
    ...
    async def test_retry_on_rate_limit(self, tavily_config):
        ...
        with patch.object(
            retriever._client, "post", new_callable=AsyncMock
        ) as mock_post:
