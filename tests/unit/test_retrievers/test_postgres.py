...
class TestPostgresRetrieverSearch:
    ...
    async def test_retrieve_sql_query(self, postgres_config, mock_pool, mock_connection):
    ...
    async def test_retrieve_text_search(self, postgres_config, mock_pool, mock_connection):
        ...
        mock_results = [
            MockRecord({"id": 1, "title": "Python Guide", "content": "Learn Python programming"}),
            MockRecord({"id": 2, "title": "Python Tutorial", "content": "Python basics"})
        ]
        mock_connection.fetchall.return_value = mock_results
    ...
    async def test_retrieve_with_filters(self, postgres_config, mock_pool, mock_connection):
    ...
    async def test_retrieve_query_error(self, postgres_config, mock_pool, mock_connection):
    ...
    async def test_retrieve_empty_results(self, postgres_config, mock_pool, mock_connection):
...
class TestPostgresRetrieverHealthCheck:
    ...
    async def test_health_check_when_connected(self, postgres_config, mock_pool, mock_connection):
    ...
    async def test_health_check_query_failure(self, postgres_config, mock_pool, mock_connection):
...
class TestPostgresRetrieverIntegration:
    ...
    async def test_execute_write_operation(self, postgres_config, mock_pool, mock_connection):
    ...
    async def test_transaction_support(self, postgres_config, mock_pool, mock_connection):
        ...
        async with retriever.transaction() as tx:
            await retriever.execute("UPDATE users SET status = 'active'")
        
        mock_connection.transaction.assert_called_once()
    
    async def test_prepared_statement(self, postgres_config, mock_pool, mock_connection):
