...
async def test_invalid_token_rejection():
    ...
    try:
        async with Client(
            "http://localhost:8001/mcp/", auth="invalid-jwt-token-12345"
        ) as client:
            # 잘못된 토큰으로 도구 호출 시도
...
async def test_expired_token_rejection():
    ...
    expired_token = jwt_service.create_access_token(
        user_id="expired-user", email="expired@example.com", roles=["user"]
    )

    try:
        async with Client("http://localhost:8001/mcp/", auth=expired_token) as client:
            result = await client.call_tool("health_check", {})
...
async def test_valid_token_acceptance():
    ...
    jwt_service = JWTService(
        secret_key=os.getenv("JWT_SECRET_KEY", "test-secret-key"), algorithm="HS256"
    )

    valid_token = jwt_service.create_access_token(
        user_id="valid-user", email="valid@example.com", roles=["user"]
    )

    try:
        async with Client("http://localhost:8001/mcp/", auth=valid_token) as client:
...
async def test_internal_api_key():
    ...
    try:
        async with Client("http://localhost:8001/mcp/", auth=internal_key) as client:
            await client.call_tool("health_check", {})
            print("✅ 내부 API 키 허용됨")
            return True
    except Exception as e:
...
async def test_wrong_internal_key_rejection():
    ...
    try:
        async with Client(
            "http://localhost:8001/mcp/", auth="wrong-internal-api-key"
        ) as client:
            result = await client.call_tool("health_check", {})
...
async def main():
    ...
    # 서버 연결 확인
    try:
        async with Client("http://localhost:8001/mcp/"):
            print("✅ MCP 서버 연결 확인")
    except Exception as e:
    ...
    critical_passed = sum(
        1 for name, success in results if name in critical_tests and success
    )
